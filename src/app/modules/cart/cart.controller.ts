...
const StoreCartProduct = async (req: Request, res: Response) => {
    try {
        ...
        const zodParsedData = CartProductValidation.StoreCartProductValidationSchema.parse(product)

        const result = await CartProductServices.storeProductCartIntoDB(zodParsedData);
